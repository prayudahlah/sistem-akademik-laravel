...
@section('content')
<div class="mb-8">
    <h1 class="text-3xl font-bold text-gray-800 mb-2">Edit Dosen</h1>
...
<form action="{{ route('dosen.update', $dosen->nip) }}" method="POST"
    ...
    <div class="mb-6">
        ...
        <textarea name="alamat"
                  id="alamat"
                  rows="3"
                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500">{{ old('alamat', $dosen->alamat) }}</textarea>
        @error('alamat')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        @enderror
    </div>
