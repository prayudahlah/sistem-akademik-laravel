...
@section('title', 'Edit Perkuliahan')

@section('content')
<div class="mb-8">
    <h1 class="text-3xl font-bold text-gray-800 mb-2">Edit Perkuliahan</h1>
...
<form action="{{ route('perkuliahan.update', $kuliah->id) }}" method="POST"
    ...
    <div class="mb-5">
        ...
        <select name="nim"
            ...
            @foreach($mahasiswa as $mhs)
                <option value="{{ $mhs->nim }}" {{ old('nim', $kuliah->nim) == $mhs->nim ? 'selected' : '' }}>
                    {{ $mhs->nim }} - {{ $mhs->nama }}
                </option>
            @endforeach
        </select>
    ...
    <div class="mb-5">
        ...
        <select name="nip"
                id="nip"
                class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 @error('nip') border-red-500 @else border-gray-300 @enderror"
                required>
            <option value="">-- Pilih Dosen --</option>
            @foreach($dosen as $dsn)
                <option value="{{ $dsn->nip }}" {{ old('nip', $kuliah->nip) == $dsn->nip ? 'selected' : '' }}>
                    {{ $dsn->nip }} - {{ $dsn->nama }}
                </option>
            @endforeach
        </select>
    ...
    <div class="mb-5">
        ...
        <select name="kode"
            ...
            @foreach($mata_kuliah as $mk)
                <option value="{{ $mk->kode }}" {{ old('kode', $kuliah->kode) == $mk->kode ? 'selected' : '' }}>
                    {{ $mk->kode }} - {{ $mk->matkul }}
                </option>
            @endforeach
        </select>
    ...
    <div class="mb-6">
        ...
        <input type="text"
               name="nilai"
               id="nilai"
               value="{{ old('nilai', $kuliah->nilai) }}"
               class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 @error('nilai') border-red-500 @else border-gray-300 @enderror"
